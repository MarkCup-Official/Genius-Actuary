import type { EChartsOption } from 'echarts'

import { graphic } from '@/lib/charts/echarts'
import type { ChartArtifact, ChartSeriesDatum } from '@/types'

const goldGradient = new graphic.LinearGradient(0, 0, 1, 1, [
  { offset: 0, color: '#F9E49F' },
  { offset: 0.5, color: '#D4AF37' },
  { offset: 1, color: '#8A7338' },
])

const sharedOption: EChartsOption = {
  animationDuration: 500,
  animationEasing: 'cubicOut',
  grid: {
    top: 56,
    right: 24,
    bottom: 36,
    left: 44,
  },
  tooltip: {
    trigger: 'axis',
    backgroundColor: '#121215',
    borderColor: 'rgba(249, 228, 159, 0.22)',
    borderWidth: 1,
    textStyle: {
      color: '#FBFBFC',
      fontSize: 12,
    },
  },
}

function axisLabelStyle() {
  return {
    color: 'rgba(251, 251, 252, 0.72)',
    fontFamily: 'JetBrains Mono',
  }
}

function buildLineOption(chart: ChartArtifact): EChartsOption {
  const points = chart.lineSeries ?? []

  return {
    ...sharedOption,
    xAxis: {
      type: 'category',
      data: points.map((point) => point.label),
      axisLabel: axisLabelStyle(),
      axisLine: { lineStyle: { color: 'rgba(255,255,255,0.08)' } },
      splitLine: { show: false },
    },
    yAxis: {
      type: 'value',
      axisLabel: axisLabelStyle(),
      splitLine: { lineStyle: { color: 'rgba(255,255,255,0.05)' } },
    },
    series: [
      {
        type: 'line',
        smooth: true,
        symbolSize: 9,
        showSymbol: true,
        lineStyle: {
          width: 3,
          color: goldGradient,
          shadowBlur: 14,
          shadowColor: 'rgba(212,175,55,0.45)',
        },
        itemStyle: {
          color: '#F9E49F',
          borderColor: '#08080A',
          borderWidth: 2,
        },
        areaStyle: {
          color: new graphic.LinearGradient(0, 0, 0, 1, [
            { offset: 0, color: 'rgba(212, 175, 55, 0.22)' },
            { offset: 1, color: 'rgba(212, 175, 55, 0.02)' },
          ]),
        },
        data: points.map((point) => point.value),
      },
    ],
  }
}

function buildBarOption(chart: ChartArtifact): EChartsOption {
  const seriesData = chart.compareSeries ?? []
  const categories = Array.from(new Set(seriesData.map((item) => item.label)))
  const groups = Array.from(new Set(seriesData.map((item) => item.group ?? 'Value')))

  return {
    ...sharedOption,
    legend: {
      textStyle: {
        color: 'rgba(251, 251, 252, 0.72)',
      },
    },
    xAxis: {
      type: 'category',
      data: categories,
      axisLabel: axisLabelStyle(),
      axisLine: { lineStyle: { color: 'rgba(255,255,255,0.08)' } },
    },
    yAxis: {
      type: 'value',
      axisLabel: axisLabelStyle(),
      splitLine: { lineStyle: { color: 'rgba(255,255,255,0.05)' } },
    },
    series: groups.map((group, index) => ({
      type: 'bar',
      name: group,
      barMaxWidth: 28,
      itemStyle: {
        borderRadius: [12, 12, 4, 4],
        color:
          index === 0
            ? goldGradient
            : 'rgba(249, 228, 159, 0.35)',
        shadowBlur: 16,
        shadowColor: 'rgba(212,175,55,0.24)',
      },
      data: categories.map((category) => {
        const datum = seriesData.find((item) => item.label === category && (item.group ?? 'Value') === group)
        return datum?.value ?? 0
      }),
    })),
  }
}

function buildScatterOption(chart: ChartArtifact): EChartsOption {
  const points = chart.scatterSeries ?? []

  return {
    ...sharedOption,
    xAxis: {
      type: 'value',
      axisLabel: axisLabelStyle(),
      splitLine: { lineStyle: { color: 'rgba(255,255,255,0.05)' } },
    },
    yAxis: {
      type: 'value',
      axisLabel: axisLabelStyle(),
      splitLine: { lineStyle: { color: 'rgba(255,255,255,0.05)' } },
    },
    series: [
      {
        type: 'scatter',
        symbolSize: (value: number[]) => Math.max(14, value[2] * 14),
        data: points.map((point) => [
          Number(point.group ?? 0),
          point.value,
          point.intensity ?? 0.6,
          point.label,
        ]),
        itemStyle: {
          color: 'rgba(249, 228, 159, 0.82)',
          shadowBlur: 26,
          shadowColor: 'rgba(212,175,55,0.48)',
        },
      },
    ],
    tooltip: {
      trigger: 'item',
      backgroundColor: '#121215',
      borderColor: 'rgba(249, 228, 159, 0.22)',
      formatter: (params) => {
        const payload = params as unknown as { data: [number, number, number, string] }
        return `${payload.data[3]}<br/>X: ${payload.data[0]}<br/>Y: ${payload.data[1]}`
      },
    },
  }
}

function buildRadarOption(chart: ChartArtifact): EChartsOption {
  const radarSeries = chart.radarSeries ?? []
  const dimensions = radarSeries[0]?.values.map((item) => item.dimension) ?? []

  return {
    ...sharedOption,
    radar: {
      indicator: dimensions.map((dimension) => ({ name: dimension, max: 10 })),
      splitLine: { lineStyle: { color: 'rgba(255,255,255,0.08)' } },
      splitArea: { areaStyle: { color: ['rgba(255,255,255,0.02)', 'rgba(255,255,255,0.01)'] } },
      axisName: {
        color: 'rgba(251,251,252,0.72)',
      },
    },
    series: [
      {
        type: 'radar',
        data: radarSeries.map((item, index) => ({
          name: item.name,
          value: item.values.map((value) => value.value),
          lineStyle: {
            color: index === 0 ? '#F9E49F' : '#D4AF37',
            width: 2,
          },
          itemStyle: {
            color: index === 0 ? '#F9E49F' : '#D4AF37',
          },
          areaStyle: {
            color: index === 0 ? 'rgba(249,228,159,0.14)' : 'rgba(212,175,55,0.08)',
          },
        })),
      },
    ],
  }
}

function buildHeatmapOption(chart: ChartArtifact): EChartsOption {
  const points = chart.heatmapSeries ?? []
  const xs = Array.from(new Set(points.map((point) => point.x)))
  const ys = Array.from(new Set(points.map((point) => point.y)))

  return {
    ...sharedOption,
    tooltip: {
      position: 'top',
    },
    xAxis: {
      type: 'category',
      data: xs,
      axisLabel: axisLabelStyle(),
    },
    yAxis: {
      type: 'category',
      data: ys,
      axisLabel: axisLabelStyle(),
    },
    visualMap: {
      min: 0,
      max: 10,
      calculable: false,
      textStyle: { color: 'rgba(251,251,252,0.72)' },
      inRange: {
        color: ['rgba(61,52,31,0.35)', '#8A7338', '#D4AF37', '#F9E49F'],
      },
    },
    series: [
      {
        type: 'heatmap',
        data: points.map((point) => [xs.indexOf(point.x), ys.indexOf(point.y), point.value]),
        label: { show: true, color: '#08080A' },
        emphasis: {
          itemStyle: {
            shadowBlur: 18,
            shadowColor: 'rgba(212,175,55,0.35)',
          },
        },
      },
    ],
  }
}

function buildPieOption(chart: ChartArtifact): EChartsOption {
  const slices = (chart.compareSeries ?? chart.lineSeries ?? []).map((item, index) => ({
    name: item.label,
    value: item.value,
    itemStyle: {
      color:
        index === 0
          ? '#F9E49F'
          : index === 1
            ? '#D4AF37'
            : index === 2
              ? '#8A7338'
              : 'rgba(249, 228, 159, 0.42)',
      borderColor: '#08080A',
      borderWidth: 2,
      shadowBlur: 18,
      shadowColor: 'rgba(212,175,55,0.22)',
    },
  }))

  return {
    ...sharedOption,
    tooltip: {
      trigger: 'item',
      backgroundColor: '#121215',
      borderColor: 'rgba(249, 228, 159, 0.22)',
      borderWidth: 1,
      textStyle: {
        color: '#FBFBFC',
        fontSize: 12,
      },
    },
    legend: {
      bottom: 0,
      textStyle: {
        color: 'rgba(251, 251, 252, 0.72)',
      },
    },
    series: [
      {
        type: 'pie',
        radius: ['44%', '72%'],
        center: ['50%', '48%'],
        label: {
          color: 'rgba(251, 251, 252, 0.72)',
          formatter: '{b}\n{d}%',
        },
        labelLine: {
          lineStyle: {
            color: 'rgba(255,255,255,0.1)',
          },
        },
        data: slices,
      },
    ],
  }
}

function hasData(chart: ChartArtifact) {
  return Boolean(
    chart.lineSeries?.length ||
      chart.compareSeries?.length ||
      chart.scatterSeries?.length ||
      chart.radarSeries?.length ||
      chart.heatmapSeries?.length,
  )
}

export function buildChartOption(chart: ChartArtifact) {
  if (!hasData(chart)) {
    return undefined
  }

  switch (chart.kind) {
    case 'line':
      return buildLineOption(chart)
    case 'bar':
      return buildBarOption(chart)
    case 'scatter':
      return buildScatterOption(chart)
    case 'radar':
      return buildRadarOption(chart)
    case 'heatmap':
      return buildHeatmapOption(chart)
    case 'pie':
      return buildPieOption(chart)
    default:
      return buildLineOption({
        ...chart,
        lineSeries: chart.lineSeries ?? [],
      })
  }
}

export function serializeChartRows(chart: ChartArtifact) {
  const series = (chart.lineSeries ??
    chart.compareSeries ??
    chart.scatterSeries ??
    chart.heatmapSeries ??
    []) as Array<ChartSeriesDatum | { x: string; y: string; value: number }>

  return series.map((row) => Object.values(row).join(' | '))
}
