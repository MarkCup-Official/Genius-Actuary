      ...
      <div className="panel-card w-full max-w-md p-8">
        <div className="mb-8 space-y-3 text-center">
          ...
          <div>
            ...
            <p className="mt-2 text-sm text-text-secondary">
              {apiMode === 'rest'
                ? 'Current backend uses anonymous cookie-based sessions. Click once to initialize the backend contract and enter the workspace.'
                : t('auth.helper')}
            </p>
          </div>
        </div>
        ...
        {apiMode === 'rest' ? (
          <div className="space-y-4">
            <div className="rounded-[22px] border border-border-subtle bg-app-bg-elevated p-4 text-sm text-text-secondary">
              The FastAPI backend exposes `/api/frontend/bootstrap` and `/api/sessions` with HTTP-only cookies. I did not find a backend API key in the repository, so this mode connects by base URL plus browser credentials.
            </div>
            ...
            >
              {mutation.isPending ? 'Connecting backend...' : 'Connect backend'}
            </Button>
          </div>
        ) : (
          <Formik
            ...
            {({ values, errors, touched, handleChange, isSubmitting }) => (
              <Form className="space-y-4">
                ...
                <Button type="submit" className="w-full" disabled={isSubmitting || mutation.isPending}>
                  {mutation.isPending ? 'Authorizing...' : t('common.confirm')}
                </Button>
              </Form>
            )}
          </Formik>
